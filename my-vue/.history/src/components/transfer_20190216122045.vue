        ...
        <div class="transfer" :style="{width,height}">
            <template v-if="mode == 'transfer'">
              ...
              <div class="transfer-left">
                <h3 class="transfer-title" style="text-align:left">
                ...
                <div class="transfer-main">
                  ...
                  <el-tree :default-checked-keys="checkedKeys" ref='from-tree' :data="self_from_data" show-checkbox :node-key="node_key" @check='fromTreeChecked' :default-expanded-keys="from_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeFrom" :default-expand-all="openAll" :render-content='renderContent' :default-checked-keys="defaultCheckedKeys">
                  </el-tree>
              ...
              <div class="transfer-right">
                ...
                <div class="transfer-main">
                  ...
                  <!-- <div class="el-transfer-panel__item">
                    <label role="checkbox" class="el-checkbox el-transfer-panel__item" aria-checked="true">
                      <span aria-checked="mixed" class="el-checkbox__input" style="left:0">
                        <span class="el-checkbox__inner"></span>
                        <input type="checkbox" aria-hidden="true" class="el-checkbox__original" value="0">
                      </span>
                      <span class="el-checkbox__label" style="text-align:left;padding-left:15px">
                        <span>上海</span>
                      </span>
                    </label>
                  </div> -->
...
export default {
  ...
  methods: {
    ...
    addToAims() {
      ...
      console.log("newSkeletonHalfCheckedNodes", newSkeletonHalfCheckedNodes)
      // 筛选到目标树不存在的骨架后在处理每个骨架节点-非末端叶子节点 - 半选节点
      newSkeletonHalfCheckedNodes.forEach(item => {
        console.log("左侧半选节点-----》", item)
        item[children__] = [];
        if (item[pid__] == 0) {
      ...
      console.log("全选节点", newSkeletonCheckedNodes)
...
作者：蒋ing 
来源：CSDN 
原文：https://blog.csdn.net/jiangjiang_ing/article/details/85163472 
版权声明：本文为博主原创文章，转载请附上博文链接！
    ...
    removeToSource() {
      ...
      let arrayCheckedNodes = this.$refs["to-tree"].getCheckedNodes(this.leafOnly);
      // 获取选中通过穿梭框的nodes - 仅用于传送选中节点数组到父组件同后台通信需求
      let nodes = JSON.parse(JSON.stringify(arrayCheckedNodes));
      console.log("you侧选中的节点",nodes)
      // 半选中节点数据
      ...
      console.log("you 侧要插入的子元素", leafCheckedNodes)
      // 末端叶子直接插入目标树
      leafCheckedNodes.forEach(item => {
        // if (!inquireIsExist(item)) {
          this.$refs["from-tree"].append(item, item[pid__]);
          console.log("左侧要插入的数据", item)
        // }
      });
      ...
      // 右侧删掉选中数据
      arrayCheckedNodes.forEach(item => {
        console.log("右侧删掉选中数据", item)
        this.$refs["to-tree"].remove(item);
        this.$refs["to-tree"].remove(item, item[pid__])
      });
      console.log("右侧删掉选中数据=====", arrayCheckedNodes)
      // 处理完毕按钮恢复禁用状态
      this.to_check_keys = [];
  ...
  computed: {
    ...
    self_to_data() {
      ...
      let new_to_array = [];
      for(let i in to_array){
          new_to_array.push(to_array[i])
      }
      return to_array;
    },
    // 左侧菜单名
    ...
    from_check_keys(val) {
      ...
      if (val.length > 0) {
        ...
        if (allCheck.length == this.self_from_data.length) {
          ...
          console.log("左边数据====", this.self_from_data) 
          this.from_is_indeterminate = true;
